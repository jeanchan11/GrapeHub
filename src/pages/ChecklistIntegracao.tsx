...
const STATUS_GROUPS: Omit<StatusGroup, 'tasks'>[] = [
  { id: 'integracao', label: 'Integração', color: '#8b5cf6', emoji: '⚙️' },
...
const TemplateModal = ({ onClose }: { onClose: () => void }) => {
  ...
  useEffect(() => {
    fetch('/api/onboarding-template?type=integracao')
      .then(r => r.ok ? r.json() : [])
...
// ── Main Component ────────────────────────────────────────
export default function ChecklistIntegracao() {
  ...
  const fetchTasks = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/onboarding-tasks?type=integracao');
      if (res.ok) {
