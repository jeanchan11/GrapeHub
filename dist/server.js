...
async function syncSicrediBillEntry(pool, billingMonth) {
  ...
  try {
    ...
    if (billRes.rows.length === 0) {
      const nb = await pool.query(`INSERT INTO fin_bills (name, category, value, recurrence, due_day, is_active) VALUES ('Cart\xE3o Sicredi','Cart\xE3o de Cr\xE9dito',NULL,'monthly',18,false) RETURNING id`);
      billId = nb.rows[0].id;
    } else billId = billRes.rows[0].id;
    const [bY, bM] = billingMonth.split("-").map(Number);
    const noteTag = `Compet\xEAncia ${String(bM).padStart(2, "0")}/${bY}`;
...
async function categorizeMovements(pool, opts = {}) {
  const norm = (s) => (s || "").toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "");
  const cats = (await pool.query("SELECT id, structure, description FROM fin_categories")).rows;
  const structId = {};
  const structDesc = {};
  for (const c of cats) {
    structId[c.structure] = c.id;
    structDesc[c.structure] = c.description;
  }
  let transferId = structId["99"];
  if (!transferId) {
    await pool.query("INSERT INTO fin_categories (external_id, structure, description, level) VALUES (999999,'99','Transfer\xEAncias entre Contas / Pagamento de Cart\xE3o',1) ON CONFLICT DO NOTHING");
  ...
  const mv = (await pool.query(
    `SELECT id, COALESCE(NULLIF(custom_description,''),description) AS desc FROM fin_movements_asaas
     WHERE is_anticipation_pair=false AND (custom_category_id IS NULL OR edited_by IN ('regra-auto','motor-auto')) ${where}`,
    params
  )).rows;
...
function setupBillsRoutes(app, pool) {
  app.get("/api/fin/bills/categories", async (req, res) => {
    try {
      ...
      const all = Array.from(/* @__PURE__ */ new Set([...DEFAULT_CATEGORIES, ...custom])).sort();
      res.json(all);
    } catch (err) {
  ...
  app.get("/api/fin/bills/entries", async (req, res) => {
    ...
    try {
      ...
      for (const bill of onceBills.rows) {
        const dueDateStr = bill.due_date instanceof Date ? bill.due_date.toISOString().slice(0, 10) : String(bill.due_date).slice(0, 10);
        await pool.query(
          `INSERT INTO fin_bill_entries (bill_id, reference_month, due_date, expected_value, status)
           VALUES ($1, $2, $3, $4, 'pending')
           ON CONFLICT (bill_id, reference_month) DO NOTHING`,
      ...
      const targetMonthPart = month.split("-")[1];
      ...
      for (const bill of yearlyBills.rows) {
        const dueDayStr = bill.due_date instanceof Date ? String(bill.due_date.getDate()).padStart(2, "0") : String(bill.due_date).split("-")[2].slice(0, 2);
      ...
      const total_previsto = rows.reduce((s, r) => s + parseFloat(r.expected_value || "0"), 0);
      const total_pago = rows.filter((r) => r.status === "paid").reduce((s, r) => s + parseFloat(r.actual_value || r.expected_value || "0"), 0);
      const total_pendente = rows.filter((r) => r.status !== "paid" && r.status !== "cancelled").reduce((s, r) => s + parseFloat(r.expected_value || "0"), 0);
      const today = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
      const vence_7_dias = rows.filter((r) => {
        if (r.status === "paid") return false;
        const diff = Math.ceil(((/* @__PURE__ */ new Date(r.due_date + "T12:00:00")).getTime() - (/* @__PURE__ */ new Date(today + "T12:00:00")).getTime()) / 864e5);
  ...
  app.post("/api/fin/bills/reconcile", async (req, res) => {
    try {
      const { reconcileBills: reconcileBills2 } = (init_asaas_sync(), __toCommonJS(asaas_sync_exports));
      const result = await reconcileBills2(pool);
  ...
  app.get("/api/fin/bills/entries/:id/candidates", async (req, res) => {
    ...
    try {
      const entry = await pool.query(
        `SELECT e.*, b.name FROM fin_bill_entries e JOIN fin_bills b ON b.id = e.bill_id WHERE e.id = $1`,
        [id]
      );
  ...
  app.post("/api/fin/bills/entries/:id/link", async (req, res) => {
    ...
    try {
      ...
      const movement = await pool.query(
        `SELECT id, value, transaction_date, description FROM fin_movements_asaas WHERE id = $1`,
        [movement_id]
      );
  ...
  app.get("/api/fin/bills/sicredi", async (req, res) => {
    ...
    try {
      ...
      const total = rows.filter((r) => r.type === -1).reduce((s, r) => s + parseFloat(r.value || "0"), 0);
      const categorized = rows.filter((r) => r.grapehub_category || r.custom_category).length;
  ...
  app.put("/api/fin/bills/sicredi/payment-date", async (req, res) => {
    try {
      ...
      if (!month) return res.status(400).json({ error: "month obrigat\xF3rio" });
      await pool.query(
...
var DEFAULT_CATEGORIES, SICREDI_AUTO_CATEGORIES, DRE_RULES;
var init_bills = __esm({
  "src/routes/bills.ts"() {
    ...
    SICREDI_AUTO_CATEGORIES = [
      ...
      { keywords: ["uber", "taxi", "99 pop"], category: "Servi\xE7os" },
      ...
      { keywords: ["folha", "salario", "rh ", "gupy", "recrutamento"], category: "Sal\xE1rios" },
    ...
    DRE_RULES = [
      ...
      { re: /taxa\s*pix/, s: "02.07.06" },
      { re: /taxa\s*boleto/, s: "02.07.07" },
      { re: /taxa.*cart/, s: "02.07.08" },
      { re: /tarifa|taxa de antecip/, s: "02.07.03" },
      { re: /\biof\b/, s: "02.07.99" },
      { re: /simples nacional/, s: "02.01.01" },
      { re: /\bdarf\b|\birpj\b|\bcsll\b/, s: "02.01.05" },
      { re: /\biss\b/, s: "02.01.06" },
      { re: /\binss\b/, s: "02.03.99" },
      { re: /\bfgts\b/, s: "02.03.07" },
      { re: /seguro/, s: "02.06.14" },
      { re: /aluguel|condominio|iptu/, s: "02.06.05" },
      { re: /energia|cpfl|enel|\bagua\b|sabesp/, s: "02.06.06" },
      { re: /internet|telefone|\btim\b|\bvivo\b|\bclaro\b/, s: "02.06.07" },
      { re: /contabil|contador/, s: "02.06.03" },
...
var asaas_sync_exports = {};
__export(asaas_sync_exports, {
  reconcileBills: () => reconcileBills,
  runAsaasSync: () => runAsaasSync,
  setupAsaasSyncRoutes: () => setupAsaasSyncRoutes
});
...
async function asaasFetchAll(endpoint, params = {}) {
...
async function asaasFetchOne(endpoint) {
  const key = getAsaasKey();
  if (!key) return null;
...
async function snapshotSaldoVirada(pool) {
  try {
    ...
    console.warn("[asaas-sync] snapshot saldo da virada:", e.message);
  }
}
async function syncPayments(pool) {
  ...
  recentFrom.setDate(today.getDate() - 7);
  ...
  const allPayments = /* @__PURE__ */ new Map();
  ...
  const custIds = [...new Set([...allPayments.values()].map((p) => p.customer).filter(Boolean))];
  const nameMap = {};
  if (custIds.length > 0) {
    try {
  ...
  try {
    ...
    if (cancelled || reconciled) console.log(`[asaas-sync] reconcilia\xE7\xE3o abertas: ${cancelled} cancelada(s), ${reconciled} atualizada(s)`);
  } catch (e) {
    console.warn("[asaas-sync] reconcilia\xE7\xE3o abertas:", e.message);
  }
  return { upserted, deleted };
}
async function syncExtrato(pool) {
  const today = /* @__PURE__ */ new Date();
  const from = new Date(today);
...
async function syncSubscriptions(pool) {
...
async function syncCustomers(pool) {
  ...
  const existingIds = new Set(existingRes.rows.map((r) => r.asaas_id));
...
async function reconcileBills(pool) {
  let matched = 0;
  try {
...
async function runAsaasSync(pool) {
  const key = getAsaasKey();
  if (!key) {
    console.warn("[asaas-sync] ASAAS_API_KEY n\xE3o configurada \u2014 sync ignorado.");
    return;
  }
  let logId = null;
  try {
  ...
  try {
    ...
    try {
      await categorizeMovements(pool);
    } catch (e) {
      console.warn("[categorizar] p\xF3s-sync:", e.message);
    }
    await snapshotSaldoVirada(pool);
    const duration = ((Date.now() - startedAt) / 1e3).toFixed(1);
    console.log(`[asaas-sync] \u2705 Conclu\xEDdo em ${duration}s \u2014 faturas: ${paymentsResult.upserted} (${paymentsResult.deleted} removidas), extrato: ${extratoResult.upserted}, assinaturas: ${subscriptionsResult.upserted}, clientes: ${customersResult.updated} (reconcilia\xE7\xE3o autom\xE1tica desativada)`);
    if (logId) {
      await pool.query(`
...
function setupAsaasSyncRoutes(app, pool) {
  pool.query(`
  ...
  `).catch((e) => console.warn("[asaas-sync] migrate log table:", e.message));
  pool.query(`
  ...
  `).then(() => {
    ...
    `);
  }).catch((e) => console.warn("[bills] migrate fin_bills/fin_bill_entries:", e.message));
  pool.query(`
  ...
  `).catch((e) => console.warn("[inadimplentes] migrate notes table:", e.message));
  pool.query(`
  ...
  `).catch((e) => console.warn("[sicredi] migrate invoice table:", e.message));
  pool.query(`
  ...
  }).catch((e) => console.warn("[bills] migrate fin_movements_asaas:", e.message));
  app.post("/api/fin/sync/run", async (_req, res) => {
    try {
  ...
  app.get("/api/fin/sync/status", async (_req, res) => {
    try {
      const r = await pool.query(`
...
async function saveDispatchComment(pool, item, mensagem, canal, contato) {
  try {
    let clientId = item.grapehub_client_id ?? null;
    ...
    if (!clientId) {
      console.warn(`[dispatch] saveDispatchComment: n\xE3o encontrou client_id para asaas_id=${item.customer_asaas_id}`);
      return;
    }
    ...
    const canalLabel = isEmail ? "\u{1F4E7} E-mail" : "\u{1F4F1} WhatsApp";
    const contatoInfo = contato || (isEmail ? item.customer_email || "" : item.customer_phone || "");
    const contatoLabel = contatoInfo ? ` \xB7 ${contatoInfo}` : "";
    await pool.query(
      `INSERT INTO crm_comments (client_id, user_id, type, content, images)
       VALUES ($1, 'sistema', 'COBRAN\xC7A', $2, '{}')`,
      [clientId, `\u{1F916} Disparo autom\xE1tico de cobran\xE7a \u2014 ${item.rule_triggered || "D" + (item.day_offset >= 0 ? "+" : "") + item.day_offset}
...
Enviado em ${now}`]
    ...
    console.log(`[dispatch] \u2705 Coment\xE1rio de cobran\xE7a salvo para client_id=${clientId}`);
  } catch (e) {
  ...
  try {
    ...
    const populated = await pool.query(`
      ...
      WHERE fr.status IN ('PENDING','OVERDUE')
        ...
        -- Evita duplicar: 1 disparo por FATURA por regra (n\xE3o apenas por cliente)
        AND NOT EXISTS (
      ...
      try {
        ...
        if (rawPhone && !normalizePhoneBR(rawPhone)) {
          await pool.query(
            ...
            [item.id, `Telefone inv\xE1lido para normaliza\xE7\xE3o: ${rawPhone}`]
          );
          console.warn(`[dispatch-scheduler] \u26A0\uFE0F Telefone inv\xE1lido: ${item.customer_name} \u2014 raw: ${rawPhone}`);
          continue;
        }
...
function setupDispatchRoutes(app, pool) {
    ...
    try {
      ...
      try {
        ...
        if (rawPhone && !normalizePhoneBR(rawPhone)) {
          await pool.query(
            `UPDATE fin_dispatch_queue SET status = 'ERRO', error_message = $2, updated_at = NOW() WHERE id = $1`,
            [id, `Telefone inv\xE1lido para normaliza\xE7\xE3o: ${rawPhone}`]
          );
          console.warn(`[dispatch] \u26A0\uFE0F Telefone inv\xE1lido (manual): ${item.customer_name} \u2014 raw: ${rawPhone}`);
          return;
        }
        const resolvedCanal = row?.canal || "Whatsapp";
        const resolvedEmail = row?.email || "";
      ...
      for (const item of items) {
        ...
        try {
          ...
          if (rawPhone && !normalizePhoneBR(rawPhone)) {
            ...
            console.warn(`[dispatch] \u26A0\uFE0F Telefone inv\xE1lido (send-all): ${item.customer_name} \u2014 raw: ${rawPhone}`);
            continue;
          }
          const resolvedCanal = row?.canal || "Whatsapp";
          const resolvedEmail = row?.email || "";
  ...
  app.post("/api/finance/dispatch/test-webhook", async (req, res) => {
    ...
    let testMessage = isEmail ? "\u{1F514} Teste GrapeHub \u2014 Esta \xE9 uma mensagem de teste de E-MAIL para validar a conectividade do webhook." : "\u{1F514} Teste GrapeHub \u2014 Esta \xE9 uma mensagem de teste de WHATSAPP para validar a conectividade do webhook.";
    try {
      ...
      console.warn("[test-webhook] Falha ao buscar template D-2, usando mensagem padr\xE3o");
    }
    const testPayload = {
...
async function setupBolaoRoutes(app, pool) {
  ...
  `);
  const seedCheck = await pool.query(`SELECT id FROM bolao.boloes WHERE nome = 'Bol\xE3o Copa 2026' LIMIT 1`);
  let bolaoSeedId;
  if (seedCheck.rowCount === 0) {
    const bolaoRes = await pool.query(
      `INSERT INTO bolao.boloes (nome, status, criado_por) VALUES ('Bol\xE3o Copa 2026', 'ativo', 'system') RETURNING id`
    );
  ...
  if (!hasOitavas) {
    ...
    const now = /* @__PURE__ */ new Date();
    const d = (days, h = 16) => {
      const dt = new Date(now.getTime() + days * 864e5);
      dt.setHours(h, 0, 0, 0);
    ...
    await pool.query(`
      ...
      ($1, 'Oitavas', 'Brasil',    'Jap\xE3o',      $2::timestamptz),
      ($1, 'Oitavas', 'Argentina', 'Pol\xF4nia',    $3::timestamptz),
      -- Pair 2 \u2192 QF2
      ($1, 'Oitavas', 'Fran\xE7a',    'B\xE9lgica',    $4::timestamptz),
      ($1, 'Oitavas', 'Alemanha',  'EUA',        $5::timestamptz),
      -- Pair 3 \u2192 QF3
      ($1, 'Oitavas', 'Portugal',  'Coreia do S',$6::timestamptz),
      ($1, 'Oitavas', 'Espanha',   'M\xE9xico',     $7::timestamptz),
      -- Pair 4 \u2192 QF4
      ($1, 'Oitavas', 'Inglaterra','Senegal',    $8::timestamptz),
      ($1, 'Oitavas', 'Holanda',   'Austr\xE1lia',  $9::timestamptz),
      -- Quartas (4 jogos)
    ...
    if (menuCheck.rowCount === 0) {
      ...
      if (grapeSection.rows.length > 0) {
        ...
        await pool.query(`
          ...
          VALUES ('bolao', $1, 'Bol\xE3o', 'Trophy', '#7C3AED', 'bolao',
            (SELECT COALESCE(MAX(order_index) + 1, 0) FROM menu_pages))
          ON CONFLICT (id) DO NOTHING
        `, [sectionId]);
        console.log("[Bolao] Menu page created in Grape section.");
      } else {
        console.log("[Bolao] Se\xE7\xE3o Grape n\xE3o encontrada \u2014 adicione o Bol\xE3o pelo Admin Panel (template: bolao).");
      }
    }
  } catch (e) {
    console.warn("[Bolao] N\xE3o foi poss\xEDvel criar menu page automaticamente:", e);
  }
  ...
  app.get("/api/bolao/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const r = await pool.query(`SELECT * FROM bolao.boloes WHERE id = $1`, [id]);
      if (r.rowCount === 0) return res.status(404).json({ error: "Bol\xE3o n\xE3o encontrado." });
      res.json(r.rows[0]);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  app.get("/api/bolao/:id/jogos", async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/bolao/:id/palpites", async (req, res) => {
    try {
      const { id } = req.params;
      const r = await pool.query(`
        SELECT
  ...
  app.post("/api/bolao/jogos/:jogoId/palpite", async (req, res) => {
    try {
      const { jogoId } = req.params;
      const uid = req.user?.uid;
      if (!uid) return res.status(401).json({ error: "N\xE3o autenticado." });
      const { palpite_casa, palpite_fora } = req.body;
      if (!Number.isInteger(palpite_casa) || !Number.isInteger(palpite_fora) || palpite_casa < 0 || palpite_fora < 0) {
        return res.status(400).json({ error: "Palpite inv\xE1lido. Gols devem ser inteiros >= 0." });
      }
      const jogoR = await pool.query(`SELECT inicia_em FROM bolao.jogos WHERE id = $1`, [jogoId]);
      if (!jogoR.rows[0]) return res.status(404).json({ error: "Jogo n\xE3o encontrado." });
      if (new Date(jogoR.rows[0].inicia_em) <= /* @__PURE__ */ new Date()) {
        return res.status(403).json({ error: "Jogo j\xE1 iniciado. Palpite bloqueado." });
      }
      const r = await pool.query(`
  ...
  app.put("/api/bolao/jogos/:jogoId/resultado", async (req, res) => {
    try {
      const uid = req.user?.uid;
      ...
      if (!Number.isInteger(gols_casa) || !Number.isInteger(gols_fora) || gols_casa < 0 || gols_fora < 0) {
        return res.status(400).json({ error: "Resultado inv\xE1lido. Gols devem ser inteiros >= 0." });
      }
      const r = await pool.query(`
        UPDATE bolao.jogos SET gols_casa = $1, gols_fora = $2, status = 'encerrado'
        WHERE id = $3 RETURNING *
      `, [gols_casa, gols_fora, jogoId]);
      if (r.rowCount === 0) return res.status(404).json({ error: "Jogo n\xE3o encontrado." });
      res.json(r.rows[0]);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  app.post("/api/bolao/:id/jogos", async (req, res) => {
    try {
      const uid = req.user?.uid;
      if (!uid || !await isAdmin(req.user)) {
        return res.status(403).json({ error: "Acesso restrito a administradores." });
      ...
      for (const jogo of body) {
        ...
        if (!fase || !time_casa || !time_fora || !inicia_em) {
          return res.status(400).json({ error: "fase, time_casa, time_fora e inicia_em s\xE3o obrigat\xF3rios." });
        }
        const r = await pool.query(`
  ...
  app.put("/api/bolao/jogos/:jogoId", async (req, res) => {
    try {
      const uid = req.user?.uid;
      if (!uid || !await isAdmin(req.user)) {
        return res.status(403).json({ error: "Acesso restrito a administradores." });
  ...
  app.get("/api/bolao/:id/ranking", async (req, res) => {
    try {
      const { id } = req.params;
      const r = await pool.query(`
        SELECT
  ...
  const bolaoAvatarUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 20 * 1024 * 1024 } });
  app.post("/api/bolao/avatar/:id", bolaoAvatarUpload.single("file"), async (req, res) => {
    try {
      const { id } = req.params;
      const file = req.file;
  ...
  app.delete("/api/bolao/avatar/:id", async (req, res) => {
    try {
      const { id } = req.params;
...
var PUBLIC_ROUTES = [
  ...
  { pattern: /^\/api\/crm-comercial\/migrate-tasks-responsible$/, method: "POST" },
  // TEMP MIGRATION
  { pattern: /^\/api\/lista-/ },
  { pattern: /^\/api\/meta-thumb$/, method: "GET" }
  // proxy de thumbnail do Meta (host restrito a fbcdn) — usado na rasterização do relatório PDF
];
...
function asaasFetch(endpoint) {
  ...
  return new Promise((resolve) => {
    const req = https2.request(`https://api.asaas.com/v3${endpoint}`, {
      method: "GET",
      headers: { "access_token": key }
    }, (res) => {
      let data = "";
...
async function startServer() {
  ...
  poolConfig.max = 10;
  poolConfig.idleTimeoutMillis = 3e4;
  poolConfig.connectionTimeoutMillis = 8e3;
  poolConfig.keepAlive = true;
  poolConfig.keepAliveInitialDelayMillis = 1e4;
  if (dbUrl) {
    try {
  ...
  function maskToken(encryptedStr) {
    try {
  ...
  try {
    ...
    await pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS collaborator_pdi (
        ...
        descricao TEXT,
        tipo TEXT CHECK (tipo IN ('Curso','Pr\xE1tica','T\xE9cnico','Comportamental')),
        prazo DATE,
        status TEXT DEFAULT 'Em andamento' CHECK (status IN ('Em andamento','Conclu\xEDdo','Pausado')),
      ...
      -- Or\xE7amento (Or\xE7ado vs Realizado): valor planejado por categoria (plano de contas) x m\xEAs
      ...
      -- Override por-inst\xE2ncia: permite editar a conta LAN\xC7ADA (parcela do m\xEAs) sem alterar
      ...
      -- Marca a parcela como editada \xE0 m\xE3o: edi\xE7\xF5es na conta cadastrada N\xC3O sobrescrevem estas.
      ...
      -- Guardamos os bytes por ad_id na 1\xAA vez que a URL ainda est\xE1 v\xE1lida; depois nunca mais quebra.
      ...
      -- Status/saldo da conta de an\xFAncios do Meta (cache) + configura\xE7\xE3o e estado dos alertas.
      ...
      -- Sync di\xE1rio do Meta Ads (migrado do n8n): config + estado da \xFAltima execu\xE7\xE3o.
      ...
      -- Webhook \xFAnico de alertas: GrapeHub empurra erros/alertas; o n8n formata e envia (WhatsApp).
          ...
          try {
            if (actionType === "create_task") {
              ...
              try {
                ...
                console.warn("[Automa\xE7\xE3o] create_onboarding: falha ao buscar reuni\xE3o do lead:", e.message);
              }
              ...
              if (existingOnboarding.rows.length > 0) {
                ...
                console.log(`[Automa\xE7\xE3o] create_onboarding: tarefa j\xE1 existia para "${clientName}", atualizada (produto/reuni\xE3o se faltavam)`);
              } else {
                const result = await pool.query(
  ...
  app.get("/api/squad-members/:subsessionId", async (req, res) => {
    try {
      const { subsessionId } = req.params;
      const result = await pool.query(`
      ...
      `, [subsessionId]);
      res.json(result.rows.map((r) => r.responsible));
    } catch (e) {
      ...
      const projects = projectsResult.rows.map((row) => ({
        ...
        churnChecklist: (() => {
          if (typeof row.churn_checklist === "string") {
            try {
              return JSON.parse(row.churn_checklist);
            } catch (e) {
              return {};
            }
          }
  ...
  app.get("/api/menu/page-subsession/:pageId", async (req, res) => {
    try {
  ...
  const taskUpload = multer2({ storage: multer2.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 } });
    ...
    try {
      ...
      const result = await pool.query(`
        ...
        SELECT
          ...
          -- Contratos SEM o conte\xFAdo do arquivo (url base64): a listagem s\xF3 precisa do nome/contagem.
          -- O arquivo completo \xE9 carregado sob demanda via GET /api/clients/:id/contracts.
  ...
  app.get("/api/clients/:id/contracts", async (req, res) => {
    try {
      ...
      let contracts = [];
      try {
        contracts = JSON.parse(r.rows[0].contracts || "[]");
      } catch {
        contracts = [];
      }
  ...
  app.post("/api/upload", express.json({ limit: "30mb" }), async (req, res) => {
    try {
      ...
      const bucket = admin2.storage().bucket(bucketName);
    ...
    try {
      const result = await pool.query(
        `SELECT id, "CLIENTE" AS cliente, "day_exit" AS day_exit, "Evitavel - inevitavel" AS tipo,
                "gestor" AS gestor, "LTV" AS ltv, "SQUAD" AS squad, "Motivo de sa\xEDda" AS motivo, "comments" AS comments
    ...
    try {
      ...
      const aPagarRes = await pool.query(`
        ...
        WHERE e.due_date >= $1 AND e.due_date < $2
          ...
          -- dedup por valor/data removido: v\xEDnculo agora \xE9 manual; o status 'paid' j\xE1 exclui as pagas.
      `, [inicio, fim]);
  ...
  app.get("/api/financeiro/dre", async (req, res) => {
    try {
      const year = req.query.year || String((/* @__PURE__ */ new Date()).getFullYear());
      ...
      const histRes = await pool.query(
        `SELECT ref_month, structure, value FROM fin_dfc_historico WHERE ref_month LIKE $1`,
        [`${year}-%`]
      );
      const hist = {};
      const histMonths = /* @__PURE__ */ new Set();
      for (const r of histRes.rows) {
        hist[`${r.ref_month}|${r.structure}`] = parseFloat(r.value);
        if (/^[0-9]/.test(r.structure)) histMonths.add(r.ref_month);
      }
      const live = {};
      const liveSemCat = {};
      ...
      const isCat = (s) => /^[0-9]+(\.[0-9]+)*$/.test(s);
      const isLvl1 = (s) => /^[0-9]+$/.test(s);
      const catRows = rowsRes.rows.filter((r) => isCat(r.structure)).map((r) => {
        const values = {};
        let total = 0;
        for (const month of months) {
      ...
      let semCatTotal = 0;
      for (const month of months) {
        const v = histMonths.has(month) ? 0 : liveSemCat[month] || 0;
        semCatValues[month] = v;
        semCatTotal += v;
      }
      const sumLvl1 = (month) => catRows.filter((r) => isLvl1(r.structure)).reduce((s, r) => s + (r.values[month] || 0), 0) + (semCatValues[month] || 0);
      const saldoInicial = {}, geracao = {}, saldoFinal = {};
      let prevFinal = null;
      for (const month of months) {
        const abertura = hist[`${month}|_saldo_abertura`];
        const histSI = hist[`${month}|_saldo_inicial`];
        saldoInicial[month] = abertura !== void 0 ? abertura : prevFinal !== null ? prevFinal : histSI !== void 0 ? histSI : 0;
      ...
      const special = (desc, src) => {
        const values = {};
        for (const m of months) values[m] = src[m] || 0;
        return { description: desc, values };
      };
      res.json({
        year,
        months,
        historicalMonths: [...histMonths].sort(),
        ...
        geracao: special("Gera\xE7\xE3o de Caixa do Per\xEDodo", geracao),
  ...
  app.get("/api/financeiro/orcamento", async (req, res) => {
    try {
      ...
      for (const r of budRes.rows) {
        (budget[r.structure] ||= {})[r.ref_month] = parseFloat(r.value);
      }
      ...
      console.error("Error building or\xE7amento:", err);
      res.status(500).json({ error: "Failed to build or\xE7amento" });
    }
  });
  app.put("/api/financeiro/orcamento", async (req, res) => {
    try {
      const items = req.body?.items || [];
      if (!Array.isArray(items)) return res.status(400).json({ error: "items inv\xE1lido" });
      let saved = 0;
      ...
      console.error("Error saving or\xE7amento:", err);
      res.status(500).json({ error: "Failed to save or\xE7amento" });
    }
  });
  app.get("/api/financeiro/indicadores", async (req, res) => {
    try {
      ...
      const refMonth = refRes.rows[0].m || (/* @__PURE__ */ new Date()).toISOString().slice(0, 7);
      const dval = async (structure) => {
      ...
      const geracaoOpRef = receitaRef + despesaOpRef;
      ...
      const geracoes = geracoesRes.rows.map((r) => parseFloat(r.value));
      ...
      res.json({
        ...
        margem_operacional: margemOp,
        margem_liquida: margemLiq,
        cac,
        novos_clientes: novosClientes,
        payback_meses: paybackMeses,
        marketing_mes: marketingRef,
        inadimplencia_valor: inadValor,
        inadimplencia_clientes: inadClientes,
        inadimplencia_pct: inadPct,
        ltv,
        ltv_dias: ltvDias,
        caixa,
        burn_medio: burnMedio,
  ...
  app.get("/api/financeiro/extrato", async (req, res) => {
    try {
      ...
      const rows = result.rows.map((r) => {
        ...
        const autoCategory2 = TRANSACTION_LABELS[r.transaction_type] || r.transaction_type;
        const catLabel = r.display_category || autoCategory2;
    ...
    try {
      ...
      if (isCSV) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          transactions.push({
            fitid: "",
            // preenchido abaixo com impressão digital estável (independe da ordem no arquivo)
      ...
      try {
        await categorizeMovements(pool, effectiveBillingMonth ? { month: effectiveBillingMonth } : {});
      } catch (e) {
  ...
  async function applyAnticipationPairing() {
    ...
    const pairsResult = await pool.query(`
      ...
      JOIN fin_movements_asaas ba
        ON (ba.description ILIKE 'Baixa da antecipacao%' OR ba.description ILIKE 'Baixa da antecipa\xE7\xE3o%')
        AND SUBSTRING(ba.description FROM 'fatura nr[.] ?([0-9]+)')
          = SUBSTRING(cr.description FROM 'fatura nr[.] ?([0-9]+)')
      WHERE (cr.description ILIKE 'Cobranca recebida%' OR cr.description ILIKE 'Cobran\xE7a recebida%')
  ...
  app.get("/api/financeiro/extrato/anticipation-stats", async (req, res) => {
    try {
      ...
      const brutoResult = await pool.query(`
        ...
        FROM fin_movements_asaas
        WHERE (description ILIKE 'Antecipacao%' OR description ILIKE 'Antecipa\xE7\xE3o%')
          AND type = 1
          AND transaction_date >= $1 AND transaction_date < $2
      `, [start, fim]);
      const taxasResult = await pool.query(`
        SELECT COALESCE(SUM(ABS(value::numeric)), 0) as total, COUNT(*) as count
        FROM fin_movements_asaas
        WHERE (description ILIKE 'Taxa de antecipacao%' OR description ILIKE 'Taxa de antecipa\xE7\xE3o%')
          AND transaction_date >= $1 AND transaction_date < $2
      `, [start, fim]);
      const pairsResult = await pool.query(`
  ...
  app.get("/api/partners/:projectId/meta-insights", async (req, res) => {
    try {
      const { projectId } = req.params;
      const today = /* @__PURE__ */ new Date();
      const fallbackStart = new Date(today);
      fallbackStart.setDate(today.getDate() - 29);
      const startStr = req.query.start || fallbackStart.toISOString().slice(0, 10);
      const endStr = req.query.end || today.toISOString().slice(0, 10);
  ...
  app.get("/api/partners/:projectId/meta-creatives", async (req, res) => {
    try {
      const { projectId } = req.params;
      const today = /* @__PURE__ */ new Date();
      const fallbackStart = new Date(today);
  ...
  app.get("/api/meta-thumb", async (req, res) => {
    const send = (mime, buf) => {
    ...
    try {
      ...
      const chunks = [];
      const upstream = https2.get(raw, (up) => {
  ...
  app.get("/api/partners/:projectId/refresh-thumbs", async (req, res) => {
    try {
      const { projectId } = req.params;
      ...
      let token;
      try {
        token = decryptToken(tok.rows[0].token_encrypted);
      } catch {
        return res.json({ cached: 0, reason: "token_error" });
      }
      const existing = await pool.query(`SELECT ad_id FROM meta_thumb_cache WHERE ad_id = ANY($1)`, [ids]);
      const have = new Set(existing.rows.map((r) => r.ad_id));
      const todo = ids.filter((id) => !have.has(id));
      const graphThumb = (adId) => new Promise((resolve) => {
        const rq = https2.get(
          ...
          (r) => {
            ...
            r.on("end", () => {
              try {
                resolve(JSON.parse(b)?.creative?.thumbnail_url || null);
              } catch {
                resolve(null);
              }
            });
      ...
      const fetchBytes = (url) => new Promise((resolve) => {
        const chunks = [];
        const rq = https2.get(url, (r) => {
  ...
  const META_STATUS = { 1: "Ativa", 2: "Desativada", 3: "N\xE3o quitada", 7: "Em an\xE1lise de risco", 8: "Aguardando pagamento", 9: "Per\xEDodo de car\xEAncia", 100: "Encerramento pendente", 101: "Encerrada", 201: "Ativa", 202: "Encerrada" };
  const META_DISABLE = { 1: "Pol\xEDtica de conte\xFAdo/integridade", 2: "An\xE1lise de IP", 3: "Pagamento / risco", 4: "Conta desativada", 5: "An\xE1lise AFC", 7: "Risco pendente" };
  async function fetchMetaAccountInfo(projectId, force = false) {
    ...
    if (!force) {
      ...
      if (c.rows.length) {
        const ttl = c.rows[0].data?.error ? 5 * 60 * 1e3 : 20 * 60 * 1e3;
    ...
    if (!tok.rows.length) return null;
    let token;
    try {
      token = decryptToken(tok.rows[0].token_encrypted);
    } catch {
      return saveCache({ error: true, errorMsg: "Token inv\xE1lido (n\xE3o foi poss\xEDvel ler)" });
    }
    let acc = String(tok.rows[0].account_id);
    ...
    const raw = await new Promise((resolve) => {
      const rq = https2.get(
        `https://graph.facebook.com/v21.0/${acc}?fields=${fields}&access_token=${encodeURIComponent(token)}`,
        (r) => {
          let b = "";
          r.on("data", (c) => b += c);
          r.on("end", () => {
    ...
    const setTokenStatus = async (s, err) => {
      await pool.query(
    ...
    if (!raw) {
      const prev = await lastGood();
      return prev && !prev.error ? prev : { error: true, transient: true, errorMsg: "Sem resposta do Meta" };
    }
    if (raw.error) {
      ...
      if (transient) {
        const prev = await lastGood();
        return prev && !prev.error ? prev : { error: true, transient: true, errorMsg: "Instabilidade no Meta" };
      }
    ...
    if (raw.account_status == null) {
      await setTokenStatus("error", "Sem acesso \xE0 conta");
      return saveCache({ error: true, errorMsg: "Sem acesso \xE0 conta" });
    }
    await setTokenStatus("active", null);
  ...
  app.get("/api/partners/:projectId/meta-account", async (req, res) => {
    try {
  ...
  async function pushAlert(payload) {
    try {
      ...
      console.warn("[alert-webhook] falhou:", e.message);
      return false;
    }
  }
  app.get("/api/alert-webhook", async (_req, res) => {
    try {
  ...
  app.put("/api/alert-webhook", async (req, res) => {
    try {
  ...
  app.post("/api/alert-webhook/test", async (_req, res) => {
    const ok = await pushAlert({ type: "teste", severity: "info", title: "Teste de webhook", message: "Se voc\xEA recebeu esta mensagem, o webhook de alertas do GrapeHub est\xE1 funcionando. \u2705" });
    res.json({ sent: ok });
  });
  app.get("/api/meta-alert-config", async (_req, res) => {
    try {
  ...
  app.put("/api/meta-alert-config", async (req, res) => {
    try {
  ...
  async function sendWhatsappAlert(webhookUrl, phone, mensagem, nome) {
    try {
      ...
      console.warn("[meta-alert] whatsapp falhou:", e.message);
    }
  }
  async function checkMetaAccounts() {
    try {
      ...
      if (!whCfg || !whCfg.enabled || !whCfg.url) return;
      ...
      for (const row of projs.rows) {
        ...
        const st = (await pool.query(`SELECT blocked_alerted, low_balance_alerted FROM meta_account_alert_state WHERE project_id=$1`, [row.project_id])).rows[0] || { blocked_alerted: false, low_balance_alerted: false };
        if (info.blocked && !st.blocked_alerted) {
          await pushAlert({
            type: "meta_account_blocked",
            severity: "error",
            partner: row.partner,
            ...
            title: `Conta de an\xFAncios com problema: ${row.partner}`,
            message: `A conta de an\xFAncios de ${row.partner} est\xE1 "${info.statusLabel}"${info.disableReason ? ` (${info.disableReason})` : ""}. As campanhas podem ter parado \u2014 verifique no Gerenciador de An\xFAncios.`
          });
        }
        const lowNow = info.prepaid && info.saldo != null && info.saldo < threshold;
        if (lowNow && !st.low_balance_alerted) {
          await pushAlert({
            type: "meta_low_balance",
            severity: "warning",
            partner: row.partner,
            squad: row.squad || null,
            saldo: info.saldo,
      ...
      console.warn("[meta-account-check]", e.message);
    }
  }
  app.post("/api/meta-account/check-now", async (_req, res) => {
    try {
      await checkMetaAccounts();
      res.json({ ok: true });
    } catch {
  ...
  const graphJson = (url, timeoutMs = 3e4) => new Promise((resolve) => {
    const rq = https2.get(url, (r) => {
      let b = "";
      r.on("data", (c) => b += c);
      r.on("end", () => {
  ...
  const sumActions = (actions, match) => (actions || []).filter((a) => a.action_type && a.action_type.includes(match)).reduce((s, a) => s + parseInt(a.value || "0", 10), 0);
  async function fetchMetaInsights(acc, level, fields, since, until, token) {
    let url = `https://graph.facebook.com/v21.0/${acc}/insights?level=${level}&fields=${encodeURIComponent(fields)}&time_increment=1&time_range=${encodeURIComponent(JSON.stringify({ since, until }))}&limit=500&access_token=${encodeURIComponent(token)}`;
    const rows = [];
  ...
  async function runMetaAdsSync() {
    ...
    try {
      ...
      for (const t of toks.rows) {
        ...
        let token;
        try {
          token = decryptToken(t.token_encrypted);
        } catch {
          await markTokenError(t.id, "Token inv\xE1lido");
          failed.push({ name: t.client_name || "?", account: t.account_id, msg: "Token inv\xE1lido", squad: t.squad || null });
          stats.errors++;
          continue;
        }
        ...
        try {
          ...
          for (const row of rows) {
            await pool.query(
              ...
              [
                ...
                sumActions(row.actions, "messaging_conversation_started")
              ]
            ).catch(() => {
            });
            stats.insights++;
          ...
          if (!isTransientMeta(e)) {
            await markTokenError(t.id, e.message);
            failed.push({ name: t.client_name || "?", account: t.account_id, msg: e.message, squad: t.squad || null });
            stats.errors++;
            continue;
          }
        }
        try {
          ...
          const uniqueAds = Array.from(new Set(rows.map((r) => r.ad_id).filter(Boolean)));
          const pickThumb = (cr) => cr?.thumbnail_url || cr?.image_url || cr?.object_story_spec?.video_data?.image_url || cr?.object_story_spec?.link_data?.picture || null;
          ...
          for (let k = 0; k < uniqueAds.length; k += 50) {
            ...
            const j = await graphJson(`https://graph.facebook.com/v21.0/?ids=${encodeURIComponent(chunk.join(","))}&fields=${thumbFields}&access_token=${encodeURIComponent(token)}`, 3e4);
            if (j && !j.error) {
      ...
      console.log(`[meta-sync] conclu\xEDdo \u2014 ${result}`);
      return stats;
    } catch (e) {
  ...
  app.get("/api/meta-sync/config", async (_req, res) => {
    try {
  ...
  app.put("/api/meta-sync/config", async (req, res) => {
    try {
  ...
  app.post("/api/meta-sync/run", async (_req, res) => {
  ...
  const api4comRequest = (method, path2, token, body) => {
    return new Promise((resolve, reject) => {
      ...
      const req = https2.request(options, (resp) => {
        let rawData = "";
        resp.on("data", (chunk) => {
          rawData += chunk;
        });
  ...
  app.get("/api/superadmin/dashboard-stats", async (req, res) => {
    try {
      ...
      const reunioesRes = await pool.query(
        `SELECT
           COALESCE(SUM(CAST(COALESCE(NULLIF("REUNI\xD5ES MARCADAS", ''), '0') AS INTEGER)), 0)::int AS marcadas,
           COALESCE(SUM(CAST(COALESCE(NULLIF("REUNI\xC3O REALIZADA", ''), '0') AS INTEGER)), 0)::int AS realizadas
         FROM reunioes
         WHERE day LIKE $1 || '%'`,
        [currentMonth]
      );
  ...
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS lista_page_settings (
        ...
        subtitle TEXT DEFAULT 'GEST\xC3O DE TAREFAS',
    ...
    console.log("lista_page_settings table initialization or migration error:", e.message);
  }
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS lista_status_groups (
        ...
        emoji TEXT DEFAULT '\u{1F535}',
        order_index INTEGER DEFAULT 0,
        PRIMARY KEY (id, page_id)
      )
    `);
  } catch (e) {
    console.log("lista_status_groups table already exists or migration skipped:", e.code);
  }
  ...
  app.get("/api/visual-hub/settings/:key", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Erro ao carregar configura\xE7\xF5es." });
    }
  });
  ...
  app.get("/api/lista-page-settings/:pageId", async (req, res) => {
    try {
      ...
      if (result.rows.length > 0) {
        ...
        res.json({ page_id: pageId, headline: "Minha Lista", subtitle: "GEST\xC3O DE TAREFAS" });
      }
    } catch (err) {
    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO lista_page_settings (page_id, headline, subtitle, notes_html, notes_html_right) VALUES ($1, $2, $3, $4, $5)
         ...
         RETURNING *`,
        [pageId, headline || "Minha Lista", subtitle || "GEST\xC3O DE TAREFAS", notes_html || "", notes_html_right || ""]
      );
      res.json(result.rows[0]);
    } catch (err) {
    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO lista_status_groups (id, page_id, label, color, emoji, order_index)
         VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
        [id, page_id, label, color || "#7c3aed", emoji || "\u{1F535}", order_index || 0]
      );
      res.json(result.rows[0]);
    } catch (err) {
  ...
  app.get("/api/onboarding-tasks", async (req, res) => {
    try {
      ...
      const rows = result.rows.map((r) => ({
        ...
        co_responsibles: (() => {
          try {
            return JSON.parse(r.co_responsibles || "[]");
          } catch {
            return [];
          }
        })(),
        time_members: (() => {
          try {
            return JSON.parse(r.time_members || "[]");
          } catch {
            return [];
          }
        })()
  ...
  app.post("/api/onboarding-tasks/:id/comments", async (req, res) => {
    try {
      ...
      if (!text?.trim() && (!files || files.length === 0)) return res.status(400).json({ error: "text ou files obrigat\xF3rio." });
      const result = await pool.query(
  ...
  app.delete("/api/onboarding-tasks/:taskId/comments/:commentId", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Erro ao excluir coment\xE1rio." });
    }
  });
  app.put("/api/onboarding-tasks/:taskId/comments/:commentId", async (req, res) => {
    try {
      ...
      if (result.rowCount === 0) return res.status(404).json({ error: "Coment\xE1rio n\xE3o encontrado." });
      res.json(result.rows[0]);
    } catch (err) {
      console.error("PUT /api/onboarding-comment error:", err);
      res.status(500).json({ error: "Erro ao editar coment\xE1rio." });
    }
  });
  ...
  app.get("/api/briefings/by-task/:taskId", async (req, res) => {
    try {
      ...
      if (taskRes.rows.length === 0) return res.status(404).json({ error: "Task n\xE3o encontrada." });
      const { client_name, nome_fantasia } = taskRes.rows[0];
      const names = [client_name, nome_fantasia].filter(Boolean);
      if (names.length === 0) return res.status(404).json({ error: "Briefing n\xE3o encontrado." });
      ...
      if (fallback.rows.length === 0) return res.status(404).json({ error: "Briefing n\xE3o encontrado." });
  ...
  app.post("/api/briefings/public/:id/submit", async (req, res) => {
    try {
      ...
      try {
        ...
        const cidadeParts = cidadeUf.split(",").map((s) => s.trim());
...
`.trim();
  ...
  app.get("/api/collaborators/:id/pdi", async (req, res) => {
    try {
      ...
      const items = rows.map((item) => {
        ...
        const concluida = item.etapas.filter((e) => e.concluida).length;
    ...
    try {
      ...
      if (Array.isArray(etapas)) {
        ...
        if (total > 0 && total === concluidas) {
          novoStatus = "Conclu\xEDdo";
        } else if (total > 0 && status === "Conclu\xEDdo" && concluidas < total) {
  ...
  app.patch("/api/pdi/etapas/:etapaId/toggle", async (req, res) => {
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        if (total > 0 && concluidas === total && currentStatus !== "Conclu\xEDdo") {
          newStatus = "Conclu\xEDdo";
        } else if (total > 0 && concluidas < total && currentStatus === "Conclu\xEDdo") {
  ...
  app.post("/api/collaborators/:id/proximo-1on1", async (req, res) => {
    ...
    if (!data) return res.status(400).json({ error: "Data \xE9 obrigat\xF3ria" });
    try {
  ...
  const LEGACY_CRITERIA = [
    ...
    { key: "nota_reunioes", criterio_id: -3, label: "Reuni\xF5es Internas", icon: "Users", cor: "blue" },
  ...
  app.get("/api/colaboradores/:id/desempenho", async (req, res) => {
    try {
      ...
      const result = rows.map((r) => {
        ...
        const media = notas.length ? Math.round(notas.reduce((a, n) => a + Number(n.nota), 0) / notas.length * 100) / 100 : 0;
  ...
  app.get("/api/colaboradores/:id/desempenho/resumo", async (req, res) => {
    try {
      ...
      const avg = (field) => {
        const sum = all.reduce((a, r) => a + Number(r[field]), 0);
      ...
      res.json({
        ...
        tendencia_campanhas: tc.t,
        diff_campanhas: tc.d,
        tendencia_grapehub: tg.t,
        diff_grapehub: tg.d,
        tendencia_reunioes: tr.t,
        diff_reunioes: tr.d,
        tendencia_tmr: tt.t,
        diff_tmr: tt.d
      });
  ...
  app.post("/api/colaboradores/:id/desempenho", async (req, res) => {
    ...
    if (!periodo_inicio || !periodo_fim) return res.status(400).json({ error: "Per\xEDodo de in\xEDcio e fim s\xE3o obrigat\xF3rios." });
    if (!Array.isArray(notas) || notas.length === 0) return res.status(400).json({ error: "Informe ao menos um crit\xE9rio avaliado." });
    try {
      const emailHeader = req.headers["x-user-email"];
      let avaliadorId = parseInt(req.params.id);
      if (emailHeader) {
        const avRes = await pool.query(`
      ...
      const cleanNotas = notas.map((n) => ({
        criterio_id: Number(n.criterio_id) || 0,
        label: String(n.label || "Avalia\xE7\xE3o"),
  ...
  app.post("/api/admin/performance-criteria", async (req, res) => {
    ...
    if (!cargo || !label) return res.status(400).json({ error: "Cargo e nome do crit\xE9rio s\xE3o obrigat\xF3rios." });
    try {
      const { rows } = await pool.query(
    ...
    try {
      const { rows } = await pool.query(
        ...
        [
          ...
          icon ?? null,
          cor ?? null,
          Number.isFinite(ordem) ? ordem : null,
          typeof ativo === "boolean" ? ativo : null
        ]
      );
      if (rows.length === 0) return res.status(404).json({ error: "Crit\xE9rio n\xE3o encontrado." });
      res.json(rows[0]);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  const FALLBACK_CRITERIO = { id: 0, label: "Avalia\xE7\xE3o", descricao: null, icon: "Star", cor: "violet" };
  ...
  app.post("/api/colaboradores/:id/pulso-diario", async (req, res) => {
    try {
      ...
      if (!["otimo", "bem", "ok", "dificil", "pesado"].includes(humor)) {
        return res.status(400).json({ error: "Humor inv\xE1lido. Use: otimo, bem, ok, dificil, pesado" });
      }
      const { rows } = await pool.query(
  ...
  async function resolveCollaboratorId(pool2, email) {
  ...
  async function getSubordinateTree(pool2, gestorCollabId) {
    const chartRes = await pool2.query(`SELECT data FROM org_chart_state WHERE id = 1`);
  ...
  app.get("/api/colaboradores/minha-equipe", async (req, res) => {
    try {
      ...
      const pulsoMap = new Map(pulsos.map((p) => [p.collaborator_id, p]));
      ...
      const perfMap = new Map(perfs.map((p) => [p.collaborator_id, parseFloat(p.media_geral)]));
      const liderados = collabs.map((c) => {
      ...
      const mediaDesempenho = perfs.length > 0 ? parseFloat((perfs.reduce((sum, p) => sum + parseFloat(p.media_geral), 0) / perfs.length).toFixed(2)) : null;
      res.json({
        liderados,
        resumo: {
          total: liderados.length,
          pulso_respondido: totalPulsoRespondido,
          pulso_total: liderados.filter((l) => l.status === "Efetivado").length,
  ...
  app.get("/api/colaboradores/minha-equipe/pulso-hoje", async (req, res) => {
    try {
      ...
      const pulsoMap = new Map(pulsos.map((p) => [p.collaborator_id, p]));
      const distribuicao = { otimo: 0, bem: 0, ok: 0, dificil: 0, pesado: 0 };
      for (const p of pulsos) {
        if (distribuicao[p.humor] !== void 0) distribuicao[p.humor]++;
      }
      const respostas = collabs.filter((c) => c.status === "Efetivado").map((c) => {
        const pulso = pulsoMap.get(c.id);
        return {
          id: c.id,
          name: c.name,
  ...
  app.get("/api/colaboradores/minha-equipe/agenda", async (req, res) => {
    try {
      const email = req.headers["x-user-email"];
      if (!email) return res.status(400).json({ error: "x-user-email header required" });
      const collabId = await resolveCollaboratorId(pool, email);
      ...
      for (const c of birthdatesList) {
        const dateStr = typeof c.birth_date === "string" ? c.birth_date : c.birth_date instanceof Date ? c.birth_date.toISOString() : "";
        if (dateStr) {
          ...
          if (parts.length === 3) {
            ...
            if (parseInt(m) === currentMonth) {
              ...
              schedules.push({
                ...
                horario: "",
                observacao: "Dia de celebrar! \u{1F389}"
              });
            }
          }
        }
  ...
  const server = app.listen(portNum, "0.0.0.0", () => {
    ...
    setTimeout(() => runAsaasSync(pool), 3 * 60 * 1e3);
    setInterval(() => runAsaasSync(pool), 10 * 60 * 1e3);
  });
}
