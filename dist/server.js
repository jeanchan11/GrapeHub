...
async function runDailyBatchIfNeeded(pool) {
  ...
  isBatchRunning = true;
  console.log(`[dispatch-scheduler] Iniciando batch di\xE1rio para ${today}`);
  try {
    await pool.query(
...
async function startServer() {
  ...
  const globalLimiter = rate_limit_default({
    windowMs: 15 * 60 * 1e3,
    // 15 minutos
    max: 200,
      ...
      await pool.query(
        `INSERT INTO todos (id, title, description, status, created_by, assigned_to, due_date, project_id, priority, subtasks, page_id) 
         ...
         ON CONFLICT (id) DO UPDATE SET 
           ...
           page_id = EXCLUDED.page_id`,
        [taskId, title, description, status || "pending", createdBy, assignedTo, dueDate, finalProjectId, priority || "M\xE9dia", JSON.stringify(subtasks || []), page_id]
      );
      if (!id) {
        await pool.query(
      ...
      const result = await Promise.all(metas.rows.map(async (meta) => {
        ...
        try {
          if (meta.tipo === "negocios_adicionados") {
            ...
            const agg = meta.metrica === "valor" ? "COALESCE(SUM(l.valor), 0)" : "COUNT(*)";
            const q = await pool.query(
              `SELECT ${agg} AS total
               FROM crm_comercial_leads l
